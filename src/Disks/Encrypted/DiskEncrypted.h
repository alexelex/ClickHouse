...
/**
 alexelex  add description
 */

class DiskEncrypted : public IDisk
{
public:
    DiskEncrypted(const String & name_, const DiskPtr & disk_, const String & key_)
        : name(name_), wrapped_disk(disk_), key(key_)
    {
    }
    ...
    // alexelex think about size. Size - sizeof(IV) or just size?
    ...
    // alexelex should I add something? May be to read another way?
    DiskDirectoryIteratorPtr iterateDirectory(const String & path) override { wrapped_disk->iterateDirectory(path); }
...
private:
    String name;
    DiskPtr wrapped_disk; // alexelex weak or shared?
    String key;
};
